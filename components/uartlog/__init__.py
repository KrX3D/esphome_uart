...
# Note: We inherit from both cg.Component and switch.Switch so that our component can act as a switch.
...
# (We do not include an "id" key here because cv.GenerateID() handles ID creation.)
UARTLOG_SWITCH_SCHEMA = switch.switch_schema(UartLogComponent)

# Async function to register our custom switch.
...
async def to_code(config):
    ...
    # Register the custom switch with platform "uartlog".
